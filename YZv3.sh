...
case $choice in
  ...
  5) install-P ;;
  6) start ;;
  7) echo "\033[32m初次安装请按照以下顺序进行操作：第一步，输入阿拉伯数字1，安装容器。第二步，输入阿拉伯数字2，配置环境。第三步，输入阿拉伯数字3或4安装对应的云崽版本。第四步，输入阿拉伯数字5，安装依赖。第五步，输入阿拉伯数字6，启动云崽。你学废了吗？:\033[0m" ;;
  我不会) echo "请使用您会使用的电子通讯产品安装并打开腾讯QQ登录，点击搜索栏输入3598537042，输入完成后请点击搜索，添加用户“会飞的芒果猫”并耐心等待，通过好友后，请您说出您的问题并附对应截图（您所遇到问题界面的截图），并耐心等待即可，有回复后，请您一定按照要求进行操作，谢谢配合" container ;;
  *) echo "请输入正确选项" container ;;
esac